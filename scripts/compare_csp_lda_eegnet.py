...
from src.pipelines.eegnet_pipeline import run_eegnet_for_subject, run_crossval_for_subject
from src.utils.results import save_accuracy_comparison, load_accuracy_comparison
from src.data.dataset import get_data_for_subject, get_crossval_data_for_subject
...
def run_crossval_experiment():
    results = []

    for subj in range(1, 10):
        subject_name = f"A0{subj}"

        print(f"\nRunning {subject_name} cross-validation...")

        data = get_crossval_data_for_subject(subj)
        ...
        csp_acc, eegnet_acc = run_crossval_for_subject(subj, X, y)
...
def main():
    ...
    results = run_crossval_experiment()
