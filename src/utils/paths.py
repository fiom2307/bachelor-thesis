...
def get_fbcsp_accuracy_comparison_path():
    return (
        CSP_LDA_EXPERIMENT_RESULTS_DIR
        / "model_accuracy_comparison_fbcsp.png"
    )


def get_fbcsp_lda_fold_model_path(
    subject: int,
    fold: int,
):
    """
    Return the path for one FBCSP+LDA fold model.
    """
    subject_name = get_subject_name(subject)
    ...
    return (
        subject_dir
        / (
            f"{subject_name}_fbcsp_lda_"
            f"kfold_seed{BASE_SEED}_"
            f"fold{fold}.joblib"
        )
    )